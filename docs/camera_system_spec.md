...
機種: **TP-Link Tapo C200**(購入済み・RTSP対応・パンチルト可・ONVIF対応でPhase 4のFrigateにも接続可)
RTSP URL形式: `rtsp://<RTSPユーザー>:<パスワード>@<IP>:554/stream1`(高画質) / `/stream2`(低画質360p)

| No | 機種 | 設置場所 | IPアドレス | RTSPパス | 備考 |
|----|------|---------|-----------|----------|------|
| 1 | Tapo C200 | (記入待ち) | (固定IP設定後に記入) | /stream1 | |
| 2 | Tapo C200 | (記入待ち) | (固定IP設定後に記入) | /stream1 | |
| 3 | Tapo C200 | (記入待ち) | (固定IP設定後に記入) | /stream1 | |
