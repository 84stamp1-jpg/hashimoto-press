...
- **計測器の制約**: データ出力機能なし。黒球の安定に15〜20分かかるため**持ち回りでの測定は不可**。
  測定場所ごとに1台を常設する必要がある
- **掲示物**: `安全衛生/heatstroke_poster.py`(2026-07-19収録)。
  `python heatstroke_poster.py 出力先.pdf` でA4縦1枚を生成。緊急連絡先はPDF入力フォームで、
  総務がAdobe Reader/Edgeで直接入力→保存→印刷できる(手書きも可)。
  **スクリプト内の WORKS(WBGT基準値)は `/wbgt/config` と必ず揃えること**。
  掲示物と画面で違う基準が出ると現場が混乱し、監督署の確認でも問題になる。
  完成PDFは `C:\Users\Owner\Desktop\熱中症_緊急対応手順_掲示用.pdf`(リポジトリには置かない)
