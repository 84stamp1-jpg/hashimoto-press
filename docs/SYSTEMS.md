...
### 17. 金型設計標準(HMS)(金型設計標準/)
- **目的**: 社内の金型設計標準を策定。社内基準と外注提示の両方に使う。単発・順送の両方が対象
- **構成**: 3編に分冊。共通の内容は共通編に集約し、各編は型種固有の項目だけを持つ。
  - 共通編33項目 / 単発編29項目(ワークのセット中心) / 順送編54項目(送り・ガイド、刃物・部品中心)
  - 各編の削除箇所には章末に「→ 共通編を参照」の行を1本置いてある。
    **外注へ単発編/順送編だけ渡すと成立しない**(共通編が要る)。3編セット渡し or 外注用結合版の作成は要決定
- **作り方**: ドラフトはExcel(`HMS金型設計標準_共通編/単発編/順送編.xlsx`、Desktop\金型仕様書\)。
  最終成果物はPDF/PowerPoint(未着手)。要約版のみ `プレス金型_設計組立の基礎_要約版.pdf` を作成済み
- **スクリプト**(金型設計標準/):
  - `migrate_to_kyotsu.py` — 各編から共通編へ移した項目を削除し参照行を挿入(2026-07-23実行済み)。
    削除項目は各編の `99_削除した項目` シートへ採否・備考ごと退避
  - `lap_figures.py` — ラップ形状パターン A〜H の参考図を作図(matplotlib)。
    客先の変更依頼書の手書き図がもとだが、車種・品番・承認印など固有情報は落として形状・寸法のみ描き直す。
    出力は `Desktop\金型仕様書\ラップ参考図\`。**現状は原図からの推測で、加筆用PDFを出して確認待ち**
- **⚠️ 出典の扱い**: 高木および客先(豊田鉄工)の資料は品番・工場名・承認印などの固有情報を必ず落とす。
  参考図をそのまま貼ると外注先に顧客の部品情報が渡るため不可
- **残タスク**: ①ラップ図 C〜H の形状確定(A・Bは確定) ②共通編の要検討2件
  (C1-4 材料歩留り=実績待ち、C3-6 スクリュプラグ採否=単発保留/順送不要) ③3編のPDF/PowerPoint化 ④外注提示の運用決定
