...
def relabel_共C2_6(im):
    # 表1（高木のプレス別値）は削除。上部をまとめて白塗りしてトリムで詰める。
    d = ImageDraw.Draw(im)
    for box in [(0, 0, 751, 372),           # 表1・〈表1〉・上端断片をまとめて削除
...
def relabel_共C3_21(im):
    d = ImageDraw.Draw(im)
    # 拡大グリッドで実測した位置。文字の下に白地を敷いて図の線と重ならせない。
    for box in [(0, 36, 76, 72), (482, 44, 566, 80),           # 3mm ×2
                (228, 36, 462, 82),                            # (1)刃先はプレス直
                (228, 104, 462, 302),                          # (3)逃がし＋底面 図1
                (730, 104, 784, 156), (696, 184, 918, 362),    # A / 逃がし＋底面 図2
                (1020, 88, 1206, 174),                         # (2)5'〜7'＋刃長部位
                (1010, 328, 1118, 372),                        # A部拡大
                (2, 366, 370, 414), (552, 368, 1152, 412),     # 鋼材ボックス内文字 ×2
                (218, 406, 328, 454), (790, 416, 878, 452),    # 〈図1〉〈図2〉
                (1242, 338, 1448, 388),                        # (3)オフセット量
                (1282, 392, 1352, 450),                        # ⚠18 → 消す
                (1336, 410, 1404, 446), (1399, 408, 1554, 448)]:  # 〈図3〉/座ぐりの場合
        wbox(d, *box)
    f20 = font(20)
    f18 = font(18)
    plain(d, 2, 44, '3mm', PURPLE, f20)
    plain(d, 487, 50, '3mm', PURPLE, f20)
    # 図1
    plain(d, 234, 44, '(1)刃先はプレス直', BLUE2, f20)
    plain(d, 234, 114, '(3)逃がし勾配か', RED, f20)
    plain(d, 254, 152, '座ぐりを施す事', RED, f20)
    plain(d, 234, 214, '(3)底面の肉が', GREEN, f18)
    plain(d, 234, 254, 'なくならぬよう注意', GREEN, f18)
    plain(d, 106, 378, '引張り強さ590材未満の鋼材', INK, f20)
    plain(d, 234, 416, '〈図1〉', INK, f20)
    # 図2
    plain(d, 740, 112, 'A', INK, f20)
    plain(d, 702, 194, '(3)逃がし勾配か', RED, f20)
    plain(d, 722, 232, '座ぐりを施す事', RED, f20)
    plain(d, 702, 286, '(3)底面の肉が', GREEN, f18)
    plain(d, 702, 322, 'なくならぬよう注意', GREEN, f18)
    plain(d, 1026, 100, "(2)5'〜7'の勾配", BLUE2, f20)
    plain(d, 1028, 140, '（刃長部位）', BLUE2, f20)
    plain(d, 1016, 342, 'A部拡大', INK, f20)
    plain(d, 560, 378, '引張り強さ590N/mm2以上及びステンレスの鋼材', INK, f18)
    plain(d, 798, 424, '〈図2〉', INK, f20)
    # 図3
    plain(d, 1250, 348, '(3)オフセット量≦1mm', GREEN, f18)
    plain(d, 1344, 418, '〈図3〉', INK, f20)
    plain(d, 1408, 418, '座ぐりの場合', RED, f20)
